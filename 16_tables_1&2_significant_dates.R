...
pathShare <- file.path(path.google, "../Shared drives/Urban Ecological Drought/Manuscript - Urban Drought NDVI Monitoring by Land Cover Class/tables")
...
growing_szn <-read.csv(file.path(google.drive, "Manuscript - Urban Drought NDVI Monitoring by Land Cover Class/data/growing_season_norms.csv")) #normals
...
write.csv(anoms_dates, file.path(pathShare, "significant_negative_anoms_dates.csv"), row.names=F)
...
write.csv(deriv_anoms_dates, file.path(pathShare, "significant_negative_deriv_anoms_dates.csv"), row.names=F)
